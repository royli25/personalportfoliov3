...
Drop image files here and they appear on the site's **components** tab — there is no
list to update. Files are picked up on build (`npm run dev` re-reads on each
request).

- **Formats:** .png, .jpg, .jpeg, .webp, .gif — plus .mp4, which renders as a
  muted clip that plays only while its card is the primary one in the scroll
  spotlight, and rests paused at frame 0 otherwise. Encode with
  `-movflags +faststart` (the gallery reads dimensions from the header, and
  faststart is what puts them there).
- **Live tiles:** a video can front an interactive component — see the
  `live` prop in `shell/gallery.tsx` (`01-ask-bar.mp4` → the ported Ask Bar
  in `src/components/askbar/`).
